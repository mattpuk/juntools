...
class jViz():
	...
	def get_fwinfo(self):
		...
		soup = self.parse_conf('<system>','</system>')
	...
	def getpolicies(self):
		...
		self.parse_conf('<policies>','</policies>')
		...
		for rec in recs:
			if rec.find('from-zone-name'):
				...
				if self.store.__contains__(self.key):
	...
	def create_graph(self,zone,direction,gdir='/root/bin/gviz/graphs/'):
		'''
